...
namespace YellowCounter.FileSystemState
{
    public class FileSystemStateOuter
    {
        ...
        private FileSystemState2 filesystemState2;
        private IRootDir root;

        public FileSystemStateOuter(string rootDir, string filter = "*")
            : this(rootDir, new FileSystemStateOptions2()
                  .WithFilter(filter))
        {
        }

        public FileSystemStateOuter(string rootDir, string filter, FileSystemStateOptions2 options)
            : this(rootDir, options.WithFilter(filter))
        {
        }

        public FileSystemStateOuter(string rootDir, FileSystemStateOptions2 options)
        {
            if(rootDir == null)
            ...
            this.filesystemState2 = Context.Resolve<FileSystemState2>();
        }

        public string RootDir => this.root.Folder;

        public void LoadState() => this.filesystemState2.LoadState();

        public IList<FileChange> GetChanges() => this.filesystemState2.GetChanges();
    }
}
